...
var WebGPUShaderProcessorGLSL = /** @class */ (function (_super) {
    ...
    WebGPUShaderProcessorGLSL.prototype.varyingProcessor = function (varying, isFragment, preProcessors, processingContext) {
    ...
    WebGPUShaderProcessorGLSL.prototype.attributeProcessor = function (attribute, preProcessors, processingContext) {
        this._preProcessors = preProcessors;
        ...
        if (match != null) {
            ...
            attribute = attribute.replace(match[0], "layout(location = " + location_2 + ") in " + attributeType + " " + name_2 + ";");
        }
        return attribute;
    };
    WebGPUShaderProcessorGLSL.prototype.uniformProcessor = function (uniform, isFragment, preProcessors, processingContext) {
        var _a;
        var _b;
        this._preProcessors = preProcessors;
        ...
        if (match != null) {
            ...
            if (uniformType.indexOf("sampler") === 0 || uniformType.indexOf("sampler") === 1) {
                ...
                var textureInfo = this.webgpuProcessingContext.availableTextures[name_3];
                if (!textureInfo) {
                    textureInfo = {
                ...
                var samplerInfo = this.webgpuProcessingContext.availableSamplers[samplerName];
                if (!samplerInfo) {
                    samplerInfo = {
                ...
                var sampleType = isComparisonSampler ? WebGPUConstants.TextureSampleType.Depth :
                ...
                if (!isTextureArray) {
                    arraySize = 1;
                    uniform = "layout(set = " + samplerGroupIndex + ", binding = " + samplerBindingIndex + ") uniform " + componentType + samplerType + " " + samplerName + ";\n                        layout(set = " + textureInfo.textures[0].groupIndex + ", binding = " + textureInfo.textures[0].bindingIndex + ") uniform " + textureType + " " + name_3 + "Texture;\n                        #define " + name_3 + " " + componentType + samplerFunction + "(" + name_3 + "Texture, " + samplerName + ")";
                }
                else {
                    var layouts = [];
                    ...
                    for (var i = 0; i < arraySize; ++i) {
                        ...
                        uniform += (i > 0 ? '\r\n' : '') + "#define " + name_3 + i + " " + componentType + samplerFunction + "(" + name_3 + "Texture" + i + ", " + samplerName + ")";
                    }
                    uniform = layouts.join('\r\n') + uniform;
                    this._textureArrayProcessing.push(name_3);
                }
                this.webgpuProcessingContext.availableTextures[name_3] = textureInfo;
    ...
    WebGPUShaderProcessorGLSL.prototype.uniformBufferProcessor = function (uniformBuffer, isFragment, processingContext) {
        var uboRegex = /uniform\s+(\w+)/gm;
        var match = uboRegex.exec(uniformBuffer);
        if (match != null) {
            var name_4 = match[1];
            var uniformBufferInfo = this.webgpuProcessingContext.availableBuffers[name_4];
            if (!uniformBufferInfo) {
                ...
                this.webgpuProcessingContext.availableBuffers[name_4] = uniformBufferInfo;
            }
    ...
    WebGPUShaderProcessorGLSL.prototype._generateLeftOverUBOCode = function (name, uniformBufferDescription) {
        var ubo = "layout(set = " + uniformBufferDescription.binding.groupIndex + ", binding = " + uniformBufferDescription.binding.bindingIndex + ") uniform " + name + " {\n    ";
    ...
    WebGPUShaderProcessorGLSL.prototype.finalizeShaders = function (vertexCode, fragmentCode, processingContext) {
