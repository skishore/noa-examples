...
var WebGPUShaderProcessor = /** @class */ (function () {
    ...
    WebGPUShaderProcessor.prototype._addUniformToLeftOverUBO = function (name, uniformType, preProcessors) {
        ...
        for (var i = 0; i < this.webgpuProcessingContext.leftOverUniforms.length; i++) {
    ...
    WebGPUShaderProcessor.prototype._buildLeftOverUBO = function () {
        ...
        var availableUBO = this.webgpuProcessingContext.availableBuffers[name];
        if (!availableUBO) {
            availableUBO = {
    ...
    WebGPUShaderProcessor.prototype._collectBindingNames = function () {
        ...
        for (var i = 0; i < this.webgpuProcessingContext.bindGroupLayoutEntries.length; i++) {
            var setDefinition = this.webgpuProcessingContext.bindGroupLayoutEntries[i];
            if (setDefinition === undefined) {
                this.webgpuProcessingContext.bindGroupLayoutEntries[i] = [];
                continue;
            }
            for (var j = 0; j < setDefinition.length; j++) {
                var entry = this.webgpuProcessingContext.bindGroupLayoutEntries[i][j];
                var name_1 = this.webgpuProcessingContext.bindGroupLayoutEntryInfo[i][entry.binding].name;
                var nameInArrayOfTexture = this.webgpuProcessingContext.bindGroupLayoutEntryInfo[i][entry.binding].nameInArrayOfTexture;
                if (entry) {
                    if (entry.texture || entry.externalTexture || entry.storageTexture) {
                        ...
                        this.webgpuProcessingContext.samplerNames.push(name_1);
                    }
                    else if (entry.buffer) {
                        this.webgpuProcessingContext.bufferNames.push(name_1);
                    }
                }
            }
        }
    ...
    WebGPUShaderProcessor.prototype._preCreateBindGroupEntries = function () {
        var bindGroupEntries = this.webgpuProcessingContext.bindGroupEntries;
        for (var i = 0; i < this.webgpuProcessingContext.bindGroupLayoutEntries.length; i++) {
            var setDefinition = this.webgpuProcessingContext.bindGroupLayoutEntries[i];
            var entries = [];
            for (var j = 0; j < setDefinition.length; j++) {
    ...
    WebGPUShaderProcessor.prototype._addTextureBindingDescription = function (name, textureInfo, textureIndex, dimension, format, isVertex) {
        ...
        if (!this.webgpuProcessingContext.bindGroupLayoutEntryInfo[groupIndex][bindingIndex]) {
            ...
            this.webgpuProcessingContext.bindGroupLayoutEntryInfo[groupIndex][bindingIndex] = { name: name, index: len - 1, nameInArrayOfTexture: textureName };
        }
    ...
    WebGPUShaderProcessor.prototype._addSamplerBindingDescription = function (name, samplerInfo, isVertex) {
        ...
        if (!this.webgpuProcessingContext.bindGroupLayoutEntryInfo[groupIndex][bindingIndex]) {
            var len = this.webgpuProcessingContext.bindGroupLayoutEntries[groupIndex].push({
                binding: bindingIndex,
                visibility: 0,
                sampler: {
            ...
            this.webgpuProcessingContext.bindGroupLayoutEntryInfo[groupIndex][bindingIndex] = { name: name, index: len - 1 };
        }
        bindingIndex = this.webgpuProcessingContext.bindGroupLayoutEntryInfo[groupIndex][bindingIndex].index;
        if (isVertex) {
            this.webgpuProcessingContext.bindGroupLayoutEntries[groupIndex][bindingIndex].visibility |= WebGPUConstants.ShaderStage.Vertex;
