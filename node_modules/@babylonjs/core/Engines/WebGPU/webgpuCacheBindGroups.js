                ...
                if (entry.sampler) {
                    ...
                    if (bindingInfo) {
                        ...
                        if (!sampler) {
                            if (this._engine.dbgSanityChecks) {
                                Logger.Error("Trying to bind a null sampler! entry=" + JSON.stringify(entry) + ", name=" + name_1 + ", bindingInfo=" + JSON.stringify(bindingInfo, function (key, value) { return key === 'texture' ? '<no dump>' : value; }) + ", materialContext.uniqueId=" + materialContext.uniqueId, 50);
                            }
                            continue;
                        }
                        ...
                        Logger.Error("Sampler \"" + name_1 + "\" could not be bound. entry=" + JSON.stringify(entry) + ", materialContext=" + JSON.stringify(materialContext, function (key, value) { return key === 'texture' || key === 'sampler' ? '<no dump>' : value; }), 50);
                    }
                }
                    ...
                    if (bindingInfo) {
                        ...
                        if (this._engine.dbgSanityChecks && (!hardwareTexture || !hardwareTexture.view)) {
                            Logger.Error("Trying to bind a null gpu texture! entry=" + JSON.stringify(entry) + ", name=" + name_1 + ", bindingInfo=" + JSON.stringify(bindingInfo, function (key, value) { return key === 'texture' ? '<no dump>' : value; }) + ", isReady=" + ((_j = bindingInfo.texture) === null || _j === void 0 ? void 0 : _j.isReady) + ", materialContext.uniqueId=" + materialContext.uniqueId, 50);
                            continue;
                        }
                        entries[j].resource = hardwareTexture.view;
                    }
                    else {
                        Logger.Error("Texture \"" + name_1 + "\" could not be bound. entry=" + JSON.stringify(entry) + ", materialContext=" + JSON.stringify(materialContext, function (key, value) { return key === 'texture' || key === 'sampler' ? '<no dump>' : value; }), 50);
                    }
                }
                    ...
                    if (bindingInfo) {
                        if (this._engine.dbgSanityChecks && bindingInfo.texture === null) {
                            Logger.Error("Trying to bind a null external texture! entry=" + JSON.stringify(entry) + ", name=" + name_1 + ", bindingInfo=" + JSON.stringify(bindingInfo, function (key, value) { return key === 'texture' ? '<no dump>' : value; }) + ", materialContext.uniqueId=" + materialContext.uniqueId, 50);
                            continue;
                        }
                        var externalTexture = bindingInfo.texture.underlyingResource;
                        if (this._engine.dbgSanityChecks && !externalTexture) {
                            Logger.Error("Trying to bind a null gpu external texture! entry=" + JSON.stringify(entry) + ", name=" + name_1 + ", bindingInfo=" + JSON.stringify(bindingInfo, function (key, value) { return key === 'texture' ? '<no dump>' : value; }) + ", isReady=" + ((_k = bindingInfo.texture) === null || _k === void 0 ? void 0 : _k.isReady) + ", materialContext.uniqueId=" + materialContext.uniqueId, 50);
                            continue;
                        }
