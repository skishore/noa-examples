...
var WebGPUShaderProcessorWGSL = /** @class */ (function (_super) {
    ...
    WebGPUShaderProcessorWGSL.prototype.varyingProcessor = function (varying, isFragment, preProcessors, processingContext) {
        ...
        if (match !== null) {
            ...
            var location_1;
            if (isFragment) {
                location_1 = this.webgpuProcessingContext.availableVaryings[name_1];
                ...
                location_1 = this.webgpuProcessingContext.getVaryingNextLocation(varyingType, this._getArraySize(name_1, varyingType, preProcessors)[2]);
                this.webgpuProcessingContext.availableVaryings[name_1] = location_1;
                this._varyingsWGSL.push("[[location(" + location_1 + ")]] " + name_1 + " : " + varyingType + ";");
                this._varyingsDeclWGSL.push("var<private> " + name_1 + " : " + varyingType + ";");
    ...
    WebGPUShaderProcessorWGSL.prototype.attributeProcessor = function (attribute, preProcessors, processingContext) {
        var attribRegex = /\s*attribute\s+(\S+)\s*:\s*(.+)\s*;/gm;
        var match = attribRegex.exec(attribute);
        if (match !== null) {
            var attributeType = match[2];
            var name_2 = match[1];
            var location_2 = this.webgpuProcessingContext.getAttributeNextLocation(attributeType, this._getArraySize(name_2, attributeType, preProcessors)[2]);
            this.webgpuProcessingContext.availableAttributes[name_2] = location_2;
            this.webgpuProcessingContext.orderedAttributes[location_2] = name_2;
            this._attributesWGSL.push("[[location(" + location_2 + ")]] " + name_2 + " : " + attributeType + ";");
            this._attributesDeclWGSL.push("var<private> " + name_2 + " : " + attributeType + ";");
            this._attributeNamesWGSL.push(name_2);
            attribute = "";
        }
        return attribute;
    };
    WebGPUShaderProcessorWGSL.prototype.uniformProcessor = function (uniform, isFragment, preProcessors, processingContext) {
    ...
    WebGPUShaderProcessorWGSL.prototype.textureProcessor = function (texture, isFragment, preProcessors, processingContext) {
        ...
        if (match !== null) {
            ...
            var textureInfo = this.webgpuProcessingContext.availableTextures[name_4];
            if (!textureInfo) {
                textureInfo = {
            ...
            this.webgpuProcessingContext.availableTextures[name_4] = textureInfo;
            ...
            var sampleType = isDepthTexture ? WebGPUConstants.TextureSampleType.Depth :
    ...
    WebGPUShaderProcessorWGSL.prototype.postProcessor = function (code, defines, isFragment, processingContext, engine) {
        return code;
    };
    WebGPUShaderProcessorWGSL.prototype.finalizeShaders = function (vertexCode, fragmentCode, processingContext) {
        ...
        for (var i = 0; i < this._attributeNamesWGSL.length; ++i) {
            ...
            vertexStartingCode += "  " + name_5 + " = input." + name_5 + ";\n";
        }
        var vertexEndingCode = "  output.position = " + builtInName_position + ";\n  output.position.y = -output.position.y;\n";
        for (var i = 0; i < this._varyingNamesWGSL.length; ++i) {
            var name_6 = this._varyingNamesWGSL[i];
            vertexEndingCode += "  output." + name_6 + " = " + name_6 + ";\n";
        }
        ...
        for (var i = 0; i < this._varyingNamesWGSL.length; ++i) {
            ...
            fragmentStartingCode += "  " + name_7 + " = input." + name_7 + ";\n";
        }
        var fragmentEndingCode = "  output.color = " + builtInName_FragColor + ";\n";
        if (hasFragDepth) {
    ...
    WebGPUShaderProcessorWGSL.prototype._generateLeftOverUBOCode = function (name, uniformBufferDescription) {
        var ubo = "[[block]] struct " + name + " {\n";
        for (var _i = 0, _a = this.webgpuProcessingContext.leftOverUniforms; _i < _a.length; _i++) {
    ...
    WebGPUShaderProcessorWGSL.prototype._injectStartingAndEndingCode = function (code, startingCode, endingCode) {
        if (startingCode) {
            var idx = code.indexOf("fn main");
            if (idx >= 0) {
    ...
    WebGPUShaderProcessorWGSL.prototype._processSamplers = function (code, isVertex) {
        ...
        while (true) {
            ...
            if (textureName) {
                var textureInfo = this.webgpuProcessingContext.availableTextures[textureName];
                if (textureInfo) {
                    textureInfo.autoBindSampler = true;
                }
            }
            var samplerInfo = this.webgpuProcessingContext.availableSamplers[name_8];
            if (!samplerInfo) {
                samplerInfo = {
                    binding: this.webgpuProcessingContext.getNextFreeUBOBinding(),
                    type: samplerBindingType,
                };
                this.webgpuProcessingContext.availableSamplers[name_8] = samplerInfo;
            }
            this._addSamplerBindingDescription(name_8, samplerInfo, isVertex);
            var part1 = code.substring(0, match.index);
            var insertPart = "[[group(" + samplerInfo.binding.groupIndex + "), binding(" + samplerInfo.binding.bindingIndex + ")]] ";
    ...
    WebGPUShaderProcessorWGSL.prototype._processCustomBuffers = function (code, isVertex) {
        ...
        while (true) {
            ...
            var bufferInfo = this.webgpuProcessingContext.availableBuffers[name_9];
            if (!bufferInfo) {
                ...
                this.webgpuProcessingContext.availableBuffers[name_9] = bufferInfo;
            }
            this._addBufferBindingDescription(name_9, this.webgpuProcessingContext.availableBuffers[name_9], decoration === "read_write" ? WebGPUConstants.BufferBindingType.Storage : type === "storage" ? WebGPUConstants.BufferBindingType.ReadOnlyStorage : WebGPUConstants.BufferBindingType.Uniform, isVertex);
            var groupIndex = bufferInfo.binding.groupIndex;
            var bindingIndex = bufferInfo.binding.bindingIndex;
            var part1 = code.substring(0, match.index);
            var insertPart = "[[group(" + groupIndex + "), binding(" + bindingIndex + ")]] ";
            var part2 = code.substring(match.index);
            code = part1 + insertPart + part2;
